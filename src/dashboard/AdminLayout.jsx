...
import { FiBox, FiUsers, FiBarChart2, FiShoppingBag, FiArrowLeft, FiLogOut } from 'react-icons/fi'

export default function AdminLayout() {
  const loc = useLocation()

  const navItems = [
    { to: "/admin/wines", title: "Menu", icon: <FiBox size={20} /> },
    { to: "/admin/orders", title: "Orders", icon: <FiShoppingBag size={20} /> },
    { to: "/admin/analytics", title: "Analytics", icon: <FiBarChart2 size={20} /> },
    { to: "/admin/users", title: "Access", icon: <FiUsers size={20} /> }
  ]
      ...
      >
        <div>
          <div className="mb-10 flex items-center justify-center md:justify-start md:px-4">
            <Link to="/admin" className="text-xl md:text-2xl font-display font-medium text-gold glow-gold tracking-widest hidden md:block">
              WINEVAULT
            </Link>
            <Link to="/admin" className="text-2xl font-display font-medium text-gold glow-gold md:hidden">
              WV
            </Link>
          </div>
      ...
      <main className="flex-1 min-w-0 max-w-full relative">
        <AnimatePresence mode="wait">
          <motion.div 
            ...
            className="w-full relative z-10 p-[1px]" // Slight padding to allow scrollbars
          >
            <Outlet />
          </motion.div>
        </AnimatePresence>
